...
: i=ident {
    ...
    setLocation(init, $i.start);  // Add location for NoInitialization
    assert($i.tree != null);
  ...
  (EQUALS e=expr {
    ...
    setLocation(init, $e.start);  // Add location for Initialization
    $tree = new DeclVar($t, $i.tree, init);
...
inst returns[AbstractInst tree]
    ...
    | SEMI {
        $tree = new NoOperation(); // Instruction vide
        }
...
@init {
    ...
    : if1=IF OPARENT condition=expr CPARENT OBRACE li_if=list_inst CBRACE {
      ...
      (ELSE elsif=IF OPARENT elsif_cond=expr CPARENT OBRACE elsif_li=list_inst CBRACE {
            ...
            $tree = new IfThenElse($tree.getCondition(), $tree.getThenBranch(), elseBranch);
            setLocation($tree, $if1);
        }
      )*
      (ELSE OBRACE li_else=list_inst CBRACE {
            assert($li_else.tree != null);
            $tree = new IfThenElse($tree.getCondition(), $tree.getThenBranch(), $li_else.tree);
            setLocation($tree, $if1);
        }
      )?
    ;
...
literal returns[AbstractExpr tree]
    ...
    | STRING {
        String text = $STRING.text.substring(1, $STRING.text.length() - 1);
