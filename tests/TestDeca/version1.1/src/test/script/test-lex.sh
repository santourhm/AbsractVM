...
test_lex_invalide() {
    # $1 = chemin du fichier
    if test_lex "$1" 2>&1 | grep -q -e "$1:[0-9][0-9]*:"; then
        echo -e "${GREEN}[PASS] Échec attendu détecté pour $1.${RESET}"
        ((success_count++))
    else
        echo -e "${RED}[FAIL] Erreur non détectée par test_lex pour $1.${RESET}"
        ((fail_count++))
    fi
}
...
test_lex_valid() {
    # $1 = chemin du fichier
    if test_lex "$1" 2>&1 | grep -q -e ":[0-9][0-9]*:"; then
...
echo ""
echo -e "${YELLOW}--- Début des tests lexicaux invalides ---${RESET}"

# Boucle sur tous les fichiers de test invalides
for cas_de_test in src/test/deca/lex/invalid/provided/*.deca; do
    test_lex_invalide "$cas_de_test"
done

# En-tête des tests valides
echo ""
echo -e "${YELLOW}--- Début des tests lexicaux valides ---${RESET}"

# Boucle sur tous les fichiers de test valides
for cas_de_test in src/test/deca/lex/valid/provided/*.deca; do
    test_lex_valid "$cas_de_test"
done

# Résumé final
echo ""
echo -e "${YELLOW}--- Résumé des tests lexicaux ---${RESET}"
